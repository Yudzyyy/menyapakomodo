  ...
  return (
    <section className="relative h-screen w-full overflow-hidden bg-black">
      ...
      <nav className="hidden md:block fixed top-4 left-1/2 -translate-x-1/2 z-50 transition-all duration-300">
        <div
          className={`flex items-center gap-4 px-5 py-2 rounded-full border shadow-lg transition-all duration-300 ${
            isScrolled
              ? "bg-white/95 backdrop-blur-xl border-gray-200 shadow-lime-200/50"
              : "bg-white/10 backdrop-blur-xl border-white/20 shadow-black/30"
          }`}
        >
          {navItems.map((item) => (
            <a
              key={item.label}
              href={item.href}
              className={`relative px-4 py-1.5 text-sm font-medium rounded-full transition-all duration-300 hover:scale-105 ${
                isScrolled
            ...
            >
              <span
                className={`absolute inset-0 -z-10 rounded-full opacity-0 hover:opacity-100 blur-md transition-opacity duration-300 ${
                  isScrolled
                    ? "bg-gradient-to-r from-lime-400/40 to-green-400/40"
                    : "bg-gradient-to-r from-lime-500/50 to-lime-300/50"
                }`}
              />
              {item.label}
            </a>
      ...
      {/* ================= MOBILE MENU ================= */}
      <div
        className={`md:hidden fixed top-20 right-4 z-40 w-64 rounded-2xl border shadow-xl transition-all duration-300 ${
          isScrolled
        ...
        <div className="p-4 space-y-2">
          {navItems.map((item) => (
            <a
              ...
              className={`block rounded-xl px-4 py-3 text-sm font-medium transition-all duration-300 hover:scale-105 ${
                isScrolled
